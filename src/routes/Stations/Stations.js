...
const columns = [
  ...
  },
  {
    ...
    render: record => (
      <Fragment>
        <Link className={styles.login} to={`/stations/${record.id}/equipments`}>
          站详情
        </Link>
...
@connect(({ rule, loading, station }) => ({
  rule,
  station,
  loading: loading.effects['station/fetch'],
}))
@Form.create()
export default class Stations extends PureComponent {
  state = {
    modalVisible: false,
    expandForm: false,
    selectedRows: [],
  ...
  render() {
    const { station: { stations }, loading, dispatch } = this.props;
